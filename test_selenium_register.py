...
def test_register_passwords_mismatch(driver, db_setup):
    ...
    try:
        driver.find_element(By.NAME, "check_password").send_keys(OTHER_PASSWORD)
    except:
        inputs = driver.find_elements(By.CSS_SELECTOR, "input[type='password']")
        if len(inputs) >= 2:
            inputs[1].send_keys(OTHER_PASSWORD)
...
def test_register_username_exist(driver, db_setup):
    ...
    existing_user = User(username=USERNAME, email=OTHER_EMAIL)
    ...
    try:
        driver.find_element(By.NAME, "password2").send_keys(OTHER_PASSWORD)
    except:
        inputs = driver.find_elements(By.CSS_SELECTOR, "input[type='password']")
        if len(inputs) >= 2:
            inputs[1].send_keys(OTHER_PASSWORD)
...
def test_register_email_exist(driver, db_setup):
    ...
    existing_user = User(username=OTHER_USERNAME, email=EMAIL)
    existing_user.set_password(PASSWORD)
    db.session.add(existing_user)
    db.session.commit()
